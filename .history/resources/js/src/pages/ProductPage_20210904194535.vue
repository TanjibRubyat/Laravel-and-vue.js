        ...
        <b-col sm="6" class="p-5">
		  ...
		  <b-row>
			  <b-col sm="4" v-for="(gImg, index) in productShow.gallery" :key="index">
				  <b-card><img :src="gImg" alt="" style="height: 150px"></b-card>
			  </b-col>
		  </b-row>
        </b-col>
        <b-col sm="6" class="p-5">
          <h3>{{productShow.name}}</h3>
          <h4>{{productShow.price}} Taka</h4>
          <p>{{productShow.description}} </p>
          <div>
            <input class="product-quantity" type="number" v-model.number="checkoutQuantity">
